"""en-US kids prompt pack — English translations of all Kids domain prompts.
...
This is the en-US (English) variant. All prompts instruct the LLM to
produce output in English (en-US).
"""
...
Even if the reference material is in another language, your output must be 100% English.
...
- Speak directly to the child ("you", "did you know...")
...
PLAN_DRAFT_SYSTEM = """You are a scriptwriter for a Kids YouTube Shorts channel.
Write a narration script in English (en-US) for a vertical Short
aimed at children.
...
REVISION_SYSTEM = """You are revising a narration script for a Kids YouTube Shorts channel.
...
5. Keep it in en-US. Plain text. Clean punctuation for TTS.
...
PLANNER_SYSTEM = """You are an EDITORIAL PLANNER for a Kids YouTube Shorts channel.
...
CRITIC_SYSTEM = """You are a SCRIPT CRITIC for a Kids YouTube Shorts channel.
...
2. NATURALNESS (0-100): Does it sound like someone TALKING to a child?
   - No AI-isms ("In this video we'll explore...")
...
Return JSON:
{
  ...
  "hook": "<first line of the script, the hook, in en-US — must be engaging for kids>",
...
CRITICAL — LANGUAGE:
The source text may be in another language. The extracted facts MUST be written
EXCLUSIVELY in English (en-US).

For each fact, provide:
- category: one of [educational, fun_fact, animal, science, history, nature, how_it_works, did_you_know, other]
- claim: a concise factual statement (1-3 sentences) in English (en-US), kid-friendly
...
STORY_FINDER_SYSTEM = """You are a STORY FINDER for a Kids YouTube Shorts channel.
...
Return ONLY valid JSON:
{
  "ideas": [
    {
      "title": "<engaging, kid-friendly title in English>",
